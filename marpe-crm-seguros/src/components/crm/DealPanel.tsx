...
export default function DealPanel({ dealId, stages, onClose, onUpdated }: Props) {
  ...
  // Fix 18: load status options once
  useEffect(() => {
  ...
  const s: Record<string, React.CSSProperties> = {
    panel: isMobile
      ...
      : { width: 368, borderLeft: '1px solid var(--border)', background: 'var(--bg-secondary)', display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100%', boxShadow: 'var(--shadow-lg)' },
    header: { padding: '16px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg-card)' },
    row: { display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--border-subtle)' },
    label: { fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase' as const, letterSpacing: '0.07em', fontWeight: 500 },
    value: { fontSize: 12, color: 'var(--text-primary)', fontWeight: 500, textAlign: 'right' as const, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const },
    tab: (active: boolean) => ({ padding: '10px 14px', fontSize: 12, fontWeight: active ? 600 : 500, cursor: 'pointer', color: active ? 'var(--accent-light)' : 'var(--text-muted)', background: 'none', border: 'none', borderBottom: `2px solid ${active ? 'var(--accent-light)' : 'transparent'}`, fontFamily: 'inherit', transition: 'color 0.15s, border-color 0.15s' }),
  ...
  return (
    <div style={s.panel}>
      ...
      <div style={s.header}>
        ...
        {/* Fix 17: Quick link to open inbox with this contact + deal context */}
      ...
      <div style={{ flex: 1, overflowY: activeTab === 'chat' ? 'hidden' : 'auto', padding: activeTab === 'chat' ? 0 : '12px 16px', display: 'flex', flexDirection: 'column' }}>
        {activeTab === 'chat' ? (
          ...
          <>
            ...
            {/* Fix 18: Status — dynamic options from marpe_status_options, fallback to free-text */}
            <div style={{ height: 12 }} />
            ...
            {statusOptions.length > 0 ? (
              ...
              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                <select
                  ...
                  onChange={e => {
                    ...
                    if (opt) setStatusColor(opt.color);
                  }}
                  style={{ flex: 1, padding: '8px 10px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)', fontSize: 12, outline: 'none', fontFamily: 'inherit', cursor: 'pointer' }}
                >
              ...
              /* Fallback free-text mode (while no options configured) */
              <>
                <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                  <input
                    value={statusCustom}
                ...
                <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 6 }}>
                  Configure opções de status em <a href="/config" style={{ color: 'var(--accent-light)', textDecoration: 'none' }}>Configurações</a>.
                </div>
              </>
            )}
            {/* Show selected color preview when using dynamic options */}
            {statusOptions.length > 0 && statusCustom && statusColor && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                <span style={{ width: 10, height: 10, borderRadius: '50%', background: statusColor, flexShrink: 0 }} />
                <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Cor do status</span>
              </div>
            )}
