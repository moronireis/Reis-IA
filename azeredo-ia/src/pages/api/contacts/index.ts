...
export const GET: APIRoute = async ({ locals, url }) => {
  ...
  const vendedor  = url.searchParams.get('vendedor') || ''; // F6: vendedor_principal (carteira Mercos)
  ...
  if (vendedor) countQuery = countQuery.eq('vendedor_principal', vendedor);
  ...
  let dataQuery = sb
    .from('az_contacts')
    .select('id, razao_social, nome_fantasia, cnpj, phone_primary, phones, email, cidade, estado, segmento, status, contato, tags, vendedor_principal, created_at')
  ...
  if (vendedor) dataQuery = dataQuery.eq('vendedor_principal', vendedor);
